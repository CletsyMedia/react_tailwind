...
const BlogCard = ({ date, day, title, description, image }) => {
  return (
    <div className="sm-max:flex sm-max:flex-col sm-max:items-center  ">
      <div className="w-[340px] h-[240px]  relative sm-max:w-[330px] sm-max:h-[230px]">
      ...
      <div className="flex flex-col mt-6 ml-2 gap-1 sm-max:ml-[2rem]">
        <h3 className="text-white font-normal w-[100%] text-[27px] sm-max:text-[20px]">{title}</h3>
        <p className="text-[#D4D4D4] w-[75%] text-[12px] sm-max:w-full sm-max:text-[14px]">{description}</p>
      </div>
      <div className="absolute text-[#03AEFD] w-[45px] h-[45px] bg-white flex items-center justify-center rounded-full cursor-pointer border-solid border-white hover:scale-75 bottom-[-4rem] right-[1rem] sm-max:hidden">
        <FaArrowRightLong />
      </div>
    </div>
  );
